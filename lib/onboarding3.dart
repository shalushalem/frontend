...
class _Screen3State extends State<Screen3> {
  ...
  Future<void> _onSaveContinue() async {
    ...
    try {
      final profile = context.read<ProfileController>().state;
      await context.read<AppwriteService>().upsertUserProfile(
            name: profile.name,
            username: profile.username.replaceAll('@', '').trim(),
            email: profile.email,
            phone: profile.phone,
            dob: profile.dob,
            ...
            stylePreferences: profile.styles.toList(),
          );
    } catch (e) {
      debugPrint('Onboarding users sync failed: $e');
    }
    if (!mounted) return;
