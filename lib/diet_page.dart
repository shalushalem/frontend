...
class _MainScreenState extends State<MainScreen> {
  ...
  final Map<int, String> _planDocIds = {};
  ...
  String _normalizePlanType(String value) {
    final type = value.toLowerCase().trim();
    if (type == 'weekly' || type == 'monthly') return type;
    return 'daily';
  }

  Meal _fallbackMealFromName(String name, int index) {
    const meta = <Map<String, String>>[
      {'type': 'Breakfast', 'cls': 'breakfast', 'icon': '🌅'},
      {'type': 'Lunch', 'cls': 'lunch', 'icon': '☀️'},
      {'type': 'Dinner', 'cls': 'dinner', 'icon': '🌙'},
      {'type': 'Snack', 'cls': 'snack', 'icon': '🍎'},
    ];
    final m = meta[index % meta.length];
    return Meal(
      type: m['type']!,
      name: name,
      desc: '',
  ...
  Meal _mealFromStored(dynamic raw, int index) {
    ...
    if (raw is String) {
      ...
      if (trimmed.isEmpty) {
        return _fallbackMealFromName('Meal ${index + 1}', index);
      }
      try {
      ...
      return _fallbackMealFromName(trimmed, index);
    }
    return _fallbackMealFromName('Meal ${index + 1}', index);
  }

  List<String> _serializePlanMeals(MealPlan plan) {
    final type = _normalizePlanType(plan.planType);
    final serialized = <String>[];

    if (type == 'daily' || plan.days.isEmpty) {
      for (final meal in plan.meals) {
        serialized.add(jsonEncode({'meal': meal.toJson()}));
      }
      return serialized;
    }
    ...
    for (final day in plan.days) {
      for (final meal in day.meals) {
        serialized.add(jsonEncode({'day': day.label, 'meal': meal.toJson()}));
      }
    }
  ...
  MealPlan _hydrateMealPlanFromDoc(dynamic doc, int localId) {
    final data = doc.data as Map<String, dynamic>;
    final rawMeals = data['meals'];
    final dailyMeals = <Meal>[];
    final daysMap = <String, List<Meal>>{};

    if (rawMeals is List) {
      for (int i = 0; i < rawMeals.length; i++) {
        final entry = rawMeals[i];
        dynamic parsed = entry;
        if (entry is String) {
        ...
        if (parsed is Map<String, dynamic> && parsed['meal'] != null) {
          final meal = _mealFromStored(parsed['meal'], i);
          final dayLabel = (parsed['day'] ?? '').toString().trim();
          if (dayLabel.isNotEmpty) {
            daysMap.putIfAbsent(dayLabel, () => <Meal>[]).add(meal);
          } else {
            dailyMeals.add(meal);
          }
        } else {
          dailyMeals.add(_mealFromStored(parsed, i));
        }
      }
    }

    final planType = _normalizePlanType((data['planType'] ?? 'daily').toString());
    List<DayPlan> days = [];
    if (daysMap.isNotEmpty) {
      days = daysMap.entries
          .map((e) => DayPlan(label: e.key, meals: e.value))
          .toList();
    } else if (planType != 'daily' && dailyMeals.isNotEmpty) {
      days = [DayPlan(label: 'Day 1', meals: List<Meal>.from(dailyMeals))];
    }
    ...
    return MealPlan(
      ...
      planType: planType,
      meals: List<Meal>.from(dailyMeals),
      days: days,
    );
  }
  ...
  Future<void> _loadPlansFromCloud() async {
    try {
      ...
      final loaded = <MealPlan>[];
      final docMap = <int, String>{};
      int seed = DateTime.now().millisecondsSinceEpoch;
      for (final doc in docs) {
        seed += 1;
        final localId = seed;
        final plan = _hydrateMealPlanFromDoc(doc, localId);
        loaded.add(plan);
        docMap[localId] = doc.$id;
      }
      ...
      _showSnack(
        SnackBar(
          content: const Text('Failed to load meal plans'),
  ...
  Future<void> _createPlanInCloud(MealPlan source, {bool showSavedSnack = false}) async {
    try {
      final appwrite = Provider.of<AppwriteService>(context, listen: false);
      final normalizedType = _normalizePlanType(source.planType);
      final localId = DateTime.now().microsecondsSinceEpoch;
      final localPlan = MealPlan(
        id: localId,
        name: source.name.trim().isEmpty ? 'Meal Plan' : source.name.trim(),
        desc: source.desc.trim().isEmpty ? 'Meal plan' : source.desc.trim(),
        planType: normalizedType,
        meals: List<Meal>.from(source.meals),
        days: source.days
            .map((d) => DayPlan(label: d.label, meals: List<Meal>.from(d.meals)))
            .toList(),
      );

      final created = await appwrite.createMealPlan({
        'name': localPlan.name,
        'desc': localPlan.desc,
        'planType': localPlan.planType,
        'totalCal': localPlan.totalCal,
        'meals': _serializePlanMeals(localPlan),
      });
      if (!mounted) return;

      setState(() {
        _plans.add(localPlan);
        _planDocIds[localPlan.id] = created.$id;
      });
      ...
      if (showSavedSnack) {
        _showSnack(SnackBar(
          content: Row(children: [
            ...
            Expanded(child: Text('"${localPlan.name}" ${AppLocalizations.t(context, 'diet_saved_to_plans')}', style: const TextStyle(fontWeight: FontWeight.w600))),
          ]),
          behavior: SnackBarBehavior.floating,
      ...
      _showSnack(
        SnackBar(
          content: const Text('Failed to save meal plan'),
          behavior: SnackBarBehavior.floating,
          backgroundColor: context.dSnackBg,
        ),
      );
  ...
  void _addPlan(MealPlan p) async {
    await _createPlanInCloud(p);
  }

  void _savePlanFromChat(MealPlan p) {
    _createPlanInCloud(p, showSavedSnack: true);
  }

  void _deletePlan(int id) async {
    final idx = _plans.indexWhere((p) => p.id == id);
    if (idx == -1) return;
    final removed = _plans[idx];
    final docId = _planDocIds[id];

    setState(() {
      _plans.removeAt(idx);
      _planDocIds.remove(id);
    });

    if (docId == null) return;
    try {
      await Provider.of<AppwriteService>(context, listen: false).deleteMealPlan(docId);
    } catch (_) {
      if (!mounted) return;
      setState(() {
        _plans.insert(idx, removed);
        _planDocIds[id] = docId;
      });
      _showSnack(
        SnackBar(
          content: const Text('Failed to delete meal plan'),
          behavior: SnackBarBehavior.floating,
          backgroundColor: context.dSnackBg,
        ),
      );
  ...
  void _editPlan(MealPlan updated) async {
    final normalized = MealPlan(
      id: updated.id,
      name: updated.name.trim().isEmpty ? 'Meal Plan' : updated.name.trim(),
      desc: updated.desc.trim().isEmpty ? 'Meal plan' : updated.desc.trim(),
      planType: _normalizePlanType(updated.planType),
      meals: List<Meal>.from(updated.meals),
      days: updated.days
          .map((d) => DayPlan(label: d.label, meals: List<Meal>.from(d.meals)))
          .toList(),
    );
    ...
    setState(() {
      final idx = _plans.indexWhere((p) => p.id == normalized.id);
      if (idx != -1) _plans[idx] = normalized;
    });

    final docId = _planDocIds[normalized.id];
    if (docId != null) {
      try {
        await Provider.of<AppwriteService>(context, listen: false).updateMealPlan(docId, {
          'name': normalized.name,
          'desc': normalized.desc,
          'planType': normalized.planType,
          'totalCal': normalized.totalCal,
          'meals': _serializePlanMeals(normalized),
        });
      } catch (_) {
        if (!mounted) return;
        _showSnack(
          SnackBar(
            content: const Text('Failed to update meal plan'),
            behavior: SnackBarBehavior.floating,
            backgroundColor: context.dSnackBg,
          ),
        );
    ...
    _showSnack(SnackBar(
      content: Row(children: [
        ...
        const SizedBox(width: 8),
        Expanded(child: Text('"${normalized.name}" ${AppLocalizations.t(context, 'diet_updated_successfully')}', style: const TextStyle(fontWeight: FontWeight.w600, color: Colors.white))),
      ]),
      behavior: SnackBarBehavior.floating,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(100)),
      backgroundColor: context.dSnackBg,
