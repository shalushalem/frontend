...
import 'package:myapp/life_goals.dart';
        ...
        builder: (context, controller, profileCtrl, child) {
          ...
          return MaterialApp(
            ...
            routes: {
              ...
              AppRoutes.lifeGoals: (_) => const LifeGoalsScreen(),
            },
          );
        },
      ),
...
class _AuthWrapperState extends State<AuthWrapper> {
  ...
  Future<void> _checkAuth() async {
    try {
      ...
      final profileCtrl = Provider.of<ProfileController>(
        context,
        listen: false,
      );
      final user = await appwrite.getCurrentUser();
      if (user != null) {
        profileCtrl.loadFromAccount(name: user.name, email: user.email);
        final profileDoc = await appwrite.ensureUserProfileDocument();
        if (profileDoc != null) {
          profileCtrl.loadFromUsersSchema(profileDoc.data);
        }
      }
      if (mounted) {
        setState(() {
  ...
  void _maybeNavigate() {
    ...
    final destination = _isLoggedIn == true
