...
class _WardrobeScreenState extends State<WardrobeScreen> {
  ...
  // Fetch from Appwrite using the shared app session.
  Future<void> _fetchWardrobeItems() async {
    try {
      ...
      final fetchedItems = docs.map((doc) {
        final occasionsRaw = doc['occasions'];
        ...
        return WardrobeItem(
          id: (doc['id'] ?? DateTime.now().millisecondsSinceEpoch.toString())
              .toString(),
          name: (doc['name'] ?? 'Garment').toString(),
          cat: (doc['category'] ?? 'Accessories').toString(),
          occasions: occasions,
          notes: (doc['notes'] ?? '').toString(),
          worn: doc['worn'] is int ? doc['worn'] as int : 0,
          liked: doc['liked'] == true,
          imageUrl: doc['image_url']?.toString(),
          maskedUrl: doc['masked_url']?.toString(),
        );
      }).toList();

      debugPrint("Wardrobe UI fetched ${fetchedItems.length} item(s) from Appwrite");
  ...
  Future<WardrobeItem?> _persistWardrobeItem(Map<String, dynamic> item) async {
    try {
      final rawBytes = item['imageBytes'] as Uint8List?;
      final maskedB64 = (item['maskedImageBase64'] as String?)?.trim() ?? '';
      if (rawBytes == null || maskedB64.isEmpty) {
        _showToast('Background removal failed. Please retake and try again.');
        return null;
      }
      ...
      final occasionsRaw = (item['occasions'] as List?) ?? const [];
      final occasions = occasionsRaw.map((e) => e.toString()).where((e) => e.trim().isNotEmpty).toList();

      final saved = await _backendService.saveWardrobeItem(
        userId: user.$id,
        rawImageBytes: rawBytes,
        maskedImageBase64: maskedB64,
        name: (item['name'] ?? 'Garment').toString(),
        category: (item['cat'] ?? 'Accessories').toString(),
        subCategory: (item['subCategory'] ?? 'General').toString(),
        pattern: (item['pattern'] ?? 'solid').toString(),
        colorCode: (item['colorCode'] ?? '').toString().trim().isEmpty ? null : item['colorCode'].toString(),
        occasions: occasions.isEmpty ? const ['casual', 'work', 'travel', 'party', 'daily wear'] : occasions,
      ...
      return WardrobeItem(
        id: (saved['document_id'] ?? DateTime.now().millisecondsSinceEpoch.toString()).toString(),
        name: (saved['name'] ?? item['name'] ?? 'Garment').toString(),
        cat: (saved['category'] ?? item['cat'] ?? 'Accessories').toString(),
        occasions: List<String>.from((saved['occasions'] as List?) ?? (item['occasions'] as List?) ?? const []),
        notes: (item['notes'] ?? '').toString(),
        imageBytes: rawBytes,
        imageUrl: (saved['image_url'] ?? item['imageUrl'])?.toString(),
        maskedUrl: (saved['masked_url'] ?? item['maskedUrl'])?.toString(),
        worn: item['worn'] as int? ?? 0,
        liked: item['liked'] as bool? ?? false,
      );
    } catch (e) {
  ...
  void _openAddModal() {
    ...
    showDialog(
      ...
      builder: (_) => _AddItemModal(
        onSave: (item) async {
          final savedItem = await _persistWardrobeItem(item);
          if (!mounted || savedItem == null) return;
          setState(() => _wardrobe.insert(0, savedItem));
...
class _DetectedItem {
  ...
  static String mapCategory(String raw) {
    ...
    if (s.contains('bottom') || s.contains('pant') || s.contains('trouser') ||
        s.contains('jean') || s.contains('short') || s.contains('skirt') ||
        s.contains('legging') || s.contains('palazzo')) {
      return 'Bottoms';
    }
    ...
    if (s.contains('footwear') || s.contains('shoe') || s.contains('boot') ||
        s.contains('sneaker') || s.contains('sandal') || s.contains('heel') ||
        s.contains('loafer') || s.contains('slipper') || s.contains('flat')) {
      return 'Footwear';
    }
...
class _AddItemModalState extends State<_AddItemModal>
  ...
  Future<void> _captureAndDetect() async {
    ...
    if (!_camReady || controller == null || !controller.value.isInitialized) return;
    ...
    try {
      ...
      if (!mounted) return;
      // Detach preview first, then dispose camera to avoid buildPreview on disposed controller.
      setState(() {
        _camReady = false;
        _camCtrl = null;
  ...
  Future<void> _pickGallery() async {
    try {
      ...
      if (bytesList.length == 1) {
        // Single image -> existing AI detection flow
        setState(() {
        ...
        // Multiple images -> parallel AI detection on all images
        setState(() {
          _capturedBytes = bytesList.first;
  ...
  Future<List<_DetectedItem>> _detectOneImage(Uint8List bytes) async {
    final analysis = await _backendService.analyzeImage(bytes);
    if (analysis == null) {
      throw Exception('Analyze failed');
    }

    final name = (analysis['name']?.toString().trim().isNotEmpty ?? false)
        ? analysis['name'].toString().trim()
        : 'Garment';
    final categoryRaw = analysis['category']?.toString() ?? '';
    final subCategory = analysis['sub_category']?.toString();
    final colorCode = analysis['color_code']?.toString();
    final pattern = analysis['pattern']?.toString();
    final maskedImageBase64 = analysis['masked_image_base64']?.toString().trim();
    if (maskedImageBase64 == null || maskedImageBase64.isEmpty) {
      throw Exception('masked_image_base64_missing');
    }
    ...
    return [
      _DetectedItem(
        ...
        color: colorCode,
        pattern: pattern,
        maskedImageBase64: maskedImageBase64,
        selected: true,
      ),
    ];
  }
  ...
  Future<void> _runDetection(Uint8List bytes) async {
    try {
      ...
      debugPrint('Wardrobe detect single failed: $e');
      final err = e.toString().toLowerCase();
      final msg = err.contains('bg_removal_failed')
          ? 'Background removal failed. Retake with cleaner background.'
          : err.contains('vision_timeout')
              ? 'Analysis timed out. Please retry analysis or enter details manually.'
          : err.contains('vision_analysis_failed')
              ? 'Analysis failed. Please retry analysis or enter details manually.'
          : err.contains('masked_image_base64_missing')
              ? 'Backend response is missing mask data. Please check backend URL/version and retry.'
          : err.contains('model runner has unexpectedly stopped')
              ? 'Vision model is overloaded. Please try again in a few seconds.'
          : err.contains('vision_fallback')
              ? 'Analysis failed. Please retry analysis or enter details manually.'
  ...
  Future<void> _runDetectionMulti(List<Uint8List> bytesList) async {
    try {
      ...
      debugPrint('Wardrobe detect multi failed: $e');
      final err = e.toString().toLowerCase();
      final msg = err.contains('bg_removal_failed')
          ? 'Background removal failed. Retake with cleaner background.'
          : err.contains('vision_timeout')
  ...
  Future<void> _manualSave() async {
    ...
    _toast('Please run detection first to remove background.');
  }
