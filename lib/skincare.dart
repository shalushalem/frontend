...
class _SkincareScreenState extends State<SkincareScreen>
  ...
  List<int> _dayStepsStored = [];
  List<int> _nightStepsStored = [];
  ...
  List<int> _asStepList(
    ...
    for (final item in value) {
      ...
      if (item is int) {
        ...
        step = int.tryParse(item.toString());
      }
      if (step == null || step < 0 || step >= maxSteps) continue;
      unique.add(step);
    }
  ...
  Future<void> _loadSkincareProfile() async {
    try {
      ...
      final data = doc.data;
      final daySteps = _asStepList(
        data['daySteps'],
        maxSteps: _dayRoutineKeys.length,
      );
      final nightSteps = _asStepList(
        data['nightSteps'],
        maxSteps: _nightRoutineKeys.length,
      );

      setState(() {
        _skincareDocId = doc.$id;
        _skinType = (data['skinType'] ?? '').toString();
        _concerns = _asStringList(data['concerns']);
        _dayStepsStored = daySteps;
        _nightStepsStored = nightSteps;
        _completedSteps = (_isNight ? _nightStepsStored : _dayStepsStored).toSet();
        _profileReady = true;
      });
    } catch (e) {
      debugPrint('Error loading skincare profile: $e');
      if (!mounted) return;
      setState(() {
        _profileReady = true;
      });
  ...
  Future<void> _saveSkincareProfile({
    String? skinType,
    List<String>? concerns,
    List<int>? daySteps,
    List<int>? nightSteps,
  }) async {
    if (!_profileReady || _skincareDocId == null) return;
    try {
      final appwrite = Provider.of<AppwriteService>(context, listen: false);
      await appwrite.updateSkincareProfile(
        documentId: _skincareDocId!,
        skinType: skinType,
        concerns: concerns,
        daySteps: daySteps,
      ...
      debugPrint('Error saving skincare profile: $e');
    }
  }
  ...
  void _setRoutine(bool night) {
    setState(() {
      _isNight = night;
      _completedSteps = (night ? _nightStepsStored : _dayStepsStored).toSet();
    });
  ...
  void _setSkin(String type) {
    ...
    _saveSkincareProfile(
      ...
      daySteps: List<int>.from(_dayStepsStored),
      nightSteps: List<int>.from(_nightStepsStored),
    );
  }
  ...
  void _toggleConcern(String concern) {
    ...
    _saveSkincareProfile(concerns: List<String>.from(_concerns));
  }
  ...
  void _markStep(int index) {
    ...
    setState(() {
      ...
      if (_isNight) {
        _nightStepsStored = _completedSteps.toList()..sort();
      } else {
        _dayStepsStored = _completedSteps.toList()..sort();
      }
    });
    _saveSkincareProfile(
      daySteps: List<int>.from(_dayStepsStored),
